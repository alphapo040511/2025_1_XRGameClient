...
public class CardDisplay : MonoBehaviour
{
    ...
    public void SetupCard(CardData data)
    {
        ...
        //SetupCard 메서드에서 카드 설명 텍스트에 효과 설명 추가
        ...
        foreach (var effect in cardDataCopy.additionalEffects)
        {
            switch (effect.effectType)
            {
                        ...
                        if (cardManager != null && cardManager.handCards.Count > 0)
                        {
                            ...
                            if(cardIndexCopy < cardManager.handCards.Count)
                            {
                                if(randomIndex != cardIndexCopy)
                                {
                                    ...
                                    //만약 버린 카드의 인덱스가 현재 카드의 인덱스보다 적다면 현재 카드의 인덱스를 1 감소 시켜야 함
                                    if(randomIndex < cardIndexCopy)
                                    {
                                        cardIndexCopy--;
                                    }
                ...
                case CardData.AdditionalEffectType.GainMana:                //플레이어 마나 획득
                    GameObject playerObj = GameObject.FindGameObjectWithTag("Player");      //태그를 사용하요 플레이어 캐릭터 찾기
                    if(playerObj != null)
                    {
                ...
                case CardData.AdditionalEffectType.ReduceEnemyMana:                //적 마나 감소
                    GameObject[] enemies  = GameObject.FindGameObjectsWithTag("Player");      //태그를 사용하요 플레이어 캐릭터 찾기
                    foreach(var enemy in enemies)
                    {
                        ...
                        if(enemyStats != null)
                        {
                            ...
                            Debug.Log($"{enemyStats.characterName}의 마나를 {effect.effectAmount} 감소 시켰습니다.");
                        }
                    }
                    break;
            
                case CardData.AdditionalEffectType.ReduceCardCost:      //다음 카드 비용 감소 효과 (시각적으로만 보여줌 실제 감소를 없음)
